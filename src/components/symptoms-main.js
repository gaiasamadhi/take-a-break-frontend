...
//   console.log(activity)
  const filteredbyGoal = activity.filter((a) => a.symptoms[0].name.includes(checked[0]) || a.symptoms[0].name.includes(checked[1]))
  ...
  return (
    ...
    <div className="symptoms-main-wrapper">
      ...
      <div className="symptoms-main-row-custom">
        ...
        <Link to={`/activity/${filteredbyGoal.length ? filteredbyGoal[Math.floor(Math.random()*filteredbyGoal.length)]._id:"616d66855fbae6e46f05c7aa"}`}><PrimaryButton text="Confirm goals" /></Link>
        <div className="symptoms-main-row-custom-wrapper">
          <img
            className="symptoms-main-row-custom-wrapper__image"
            src={paper}
            alt="Paper"
          />
        </div>
